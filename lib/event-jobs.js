...
EventJobs.prototype._assertExchange = function (exchangeName, cb) {
  debug('_assertExchange', exchangeName, !!this._channel);
  if (!this._channel) {
    debug('error: channel required for EventJobs:_assertExchange');
    throw new Error('channel required for EventJobs:_assertExchange');
  }
...
EventJobs.prototype._assertAndBindQueue = function (eventName, cb) {
  debug('_assertAndBindQueue', eventName, !!this._channel);
  if (!this._channel) {
    debug('error: channel required for EventJobs:_assertAndBindQueue');
    throw new Error('channel required for EventJobs:_assertAndBindQueue');
  }
  var self = this;
