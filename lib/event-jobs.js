...
EventJobs.prototype.assertExchanges = function (cb) {
  ...
  async.forEach(this._publishedEvents, this._createExchange.bind(this), cb);
};
...
EventJobs.prototype.createQueues = function (cb) {
  debug('createQueues');
  async.forEach(this._subscribedEvents, this._createQueue.bind(this), cb);
};
 ...
 * @param  {Function} cb    (err)
 */
EventJobs.prototype._createQueue = function (eventName, cb) {
  debug('_createQueue', eventName);
  var self = this;

  var queueName = [self._name, eventName, 'queue'].join('.');
...
EventJobs.prototype.subscribe = function (eventName, cb) {
  ...
  var queueName = [this._name, eventName, 'queue'].join('.');
